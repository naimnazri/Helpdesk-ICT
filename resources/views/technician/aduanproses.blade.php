        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                ...
                <li class="breadcrumb-item active">Aduan Dalam Tindakan</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                  <h3 class="card-title">Senarai Aduan Dalam Tindakan</h3>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                    <table id="tableAduan" class="table table-bordered table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $adu->jabatan }}</td>
                                <td class="text-center">@if($adu->tarikh_aduan == '')
                                    @else
                                     {{ date('d-m-Y', strtotime($adu->tarikh_tindakan))}}</td>
                                    @endif
                                </td>
                                <td><?php
                                    $diff = Carbon\Carbon::parse($adu->tarikh_tindakan)->diffInDays()
                                    ?>
                                    @if($diff > '3')
                                    <div class="text-danger text-bold text-center">{{$diff}}</div>
                                    @else
                                    <div class="text-center">{{$diff}}</div>
                                    @endif
                                </td>
                                <td>
                                    <a type="button" title="Tindakan Aduan" class="btn btn-block btn-success btn-sm"
                                        href="{{ url('t_editpembekal/'. $adu->no_aduan)}}">
                                            <i class="fas fa-edit"></i>
                                    </a>
                                    {{-- <a type="button" title="Maklumat Aduan" class="btn btn-block btn-info btn-sm"
                                        href="{{ url('t_maklumatAduan/'.$adu->id)}}">
